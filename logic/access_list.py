...
class Match():
        ...
        if match_type == "ipv4":
            ...
            if rule_type == "from":
                if constants.IPV4_FROM_DNS_NAME_KEY in match_value:
                    self.dns_name = match_value[constants.IPV4_FROM_DNS_NAME_KEY]
                    if '*' in self.dns_name:
                ...
                if constants.IPV4_TO_DNS_NAME_KEY in match_value:
                    self.dns_name = match_value[constants.IPV4_TO_DNS_NAME_KEY]
                    if '*' in self.dns_name:
                        self.is_domain_generalized = True  # checkes if the given domain is generalized (when there is a domain)
    ...
    def domain_similarity(self, a, b):
        a_domain_parts_rev = a.split('.')[::-1]
        b_domain_parts_rev = b.split('.')[::-1]
        ...
        for i in range(similarity_max_score):
            if a_domain_parts_rev[i] == b_domain_parts_rev[i]:
                similarity_score += 1
                self.generalized_domain = '.' + a_domain_parts_rev[i] + self.generalized_domain
            else:
                break
